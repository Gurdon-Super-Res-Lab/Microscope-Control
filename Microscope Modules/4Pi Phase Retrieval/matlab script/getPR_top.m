function [CN_phase,oprobj] = getPR_top(NA, lambda, RI, pixelsize,zrange,CCDoffset,gain,center,channel,pfactor,dmfile,datadir)
...
mL = zeros(1,N);
for ii = 1:N
    [n,mL(ii)] = oprobj.PRobj.Z.indices_NollLtoNM(ii);
end
mL(mL==0) = 1;
signm = mL./abs(mL);
dphase = -1.*signm.*phaseZ.*T/2/pi*pfactor;
dphase(1:4) = 0;
load('calibration files\upperDM_z2m.mat')
N1 = length(dphase);
