...
% V5 - Assume each image is a multi-page TIF

%% clear everything
clear all; clc

%% set the directory to read from and get all the TIF images
rDir = 'C:\Users\George Sirinakis\Documents\Microscope Control\Microscope Modules\Wide Field Drift Correction\WFDC DLL and Testing\Drift Correction Test Stacks 20171123\StepSize 100 nm Stack 03 - 20 Frames per Step';
...
imRef01 = imread([rDir, filesep, filer(1).name], 1);
imRef02 = imread([rDir, filesep, filer(centerIndex).name], 1);
imRef03 = imread([rDir, filesep, filer(end).name], 1);
...
stepSize = 100;
fpStep = 20;
zetaInfo = zeros(length(filer)*fpStep, 2);
zetaIndex = 1;
...
for i = 1:length(filer)
    ...
    for frameNum = 1:fpStep
        fprintf(1, 'Frame: %d\n', frameNum);
        
        currImage = imread([rDir, filesep, filer(i).name], frameNum);
        ...
        % save the results
        zetaInfo(zetaIndex, :) = [(i - 1)*stepSize, zeta];
...
fprintf(1, 'Done\n');
toc
clear zeta cPn c0n cMn PVcPn PVc0n PVcMn currImage i frameNum

%% plot the zeta info

plot(zetaInfo(:, 1), zetaInfo(:, 2), '.')
